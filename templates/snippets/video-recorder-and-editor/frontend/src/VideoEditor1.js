...
const VideoEditor = () => {
  ...
  const trimVideo = async () => {
    ...
    try {
      ...
      const startTime = timeRange[0]; // Start of the range to cut
      const endTime = timeRange[1];   // End of the range to cut
      const totalDuration = duration; // Total video duration
  
      // First part: from 0 to startTime (before the range to cut)
      const firstPartFile = 'firstPart.mp4';
      await ffmpeg.exec([
        '-ss', '0',
        '-i', inputFileName,
        '-to', String(startTime),
      ...
      // Second part: from endTime to the end of the video (after the range to cut)
      const secondPartFile = 'secondPart.mp4';
      await ffmpeg.exec([
        '-ss', String(endTime),
        '-i', inputFileName,
        '-to', String(totalDuration - endTime),
        '-c', 'copy',
        secondPartFile
      ]);
      ...
      await ffmpeg.writeFile(concatFile, `file '${firstPartFile}'\nfile '${secondPartFile}'\n`);
      ...
      // Read the resulting file and create a URL for the user to download/view
