...
const VideoRecorder = () => {
    ...
    const startScreenSharing = async () => {
        try {
            ...
            if (recording) {
                // If already recording, create a new combined stream
                const newCombinedStream = new MediaStream();
                
                // Add screen sharing tracks
                stream.getTracks().forEach(track => newCombinedStream.addTrack(track));
                
                // Add existing tracks from the current combined stream
                if (combinedStreamRef.current) {
                    combinedStreamRef.current.getTracks().forEach(track => {
                        if (track.kind !== 'video') { // Only add non-video tracks
                ...
                // Start a new recording with the combined stream
                combinedStreamRef.current = newCombinedStream;
                const newMediaRecorder = new MediaRecorder(newCombinedStream);
                ...
                // If not recording, just show the screen sharing preview
                if (videoRef.current) {
        ...
        try {
            ...
            if (useMic && selectedMicrophone) {
                try {
                    ...
                    const defaultAudioStream = await navigator.mediaDevices.getUserMedia({ audio: true });
                    combinedStream.addTrack(defaultAudioStream.getAudioTracks()[0]);
                }
            }
    ...
    const stopScreenSharing = () => {
        if (screenSharingStream) {
            ...
            // If recording is active, revert to webcam only
            if (recording && useWebcam && webcamStream) {
                const newCombinedStream = new MediaStream();
                
                // Add webcam track
                newCombinedStream.addTrack(webcamStream.getVideoTracks()[0]);
                ...
                // Start new recording with webcam only
                combinedStreamRef.current = newCombinedStream;
                const newMediaRecorder = new MediaRecorder(newCombinedStream);
                mediaRecorderRef.current = newMediaRecorder;
