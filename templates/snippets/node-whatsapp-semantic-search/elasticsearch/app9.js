...
async function generateEmbeddings(text) {
    ...
    return Array.from(result[0][0]); // Extract embedding from pipeline result
}
...
async function createIndex() {
    try {
            ...
            body: {
                ...
                mappings: {
                    properties: {
                        ...
                        Message: { type: 'text', analyzer: 'qa_analyzer' },
        ...
        console.log("Index created successfully with vector search capabilities.");
    } catch (error) {
...
async function indexDocuments() {
    try {
        ...
        if (response.errors) {
            ...
            console.log(`Successfully indexed ${dataset.length} documents with embeddings.`);
        }
    } catch (error) {
...
async function analyzeConversation(query) {
    ...
    const table = new Table({
        head: ['Score', 'Time', 'From', 'To', 'Message', 'Context', 'Topics'],
        colWidths: [8, 20, 10, 10, 30, 20, 20]
    });
    ...
    // Additional analysis
    const participants = new Set(results.map(hit => [hit._source.Sender, hit._source.Receiver]).flat());
    const topics = new Set(results.map(hit => hit._source.Topics).flat());

    console.log("\nAnalysis Summary:");
    console.log("================");
    console.log(`Participants Involved: ${Array.from(participants).join(', ')}`);
    console.log(`Topics Discussed: ${Array.from(topics).join(', ')}`);
}
...
async function main() {
    try {
        ...
        // const queries = [
        //     "Tell me about meeting schedules and time discussions",
        //     "Find conversations about agenda and preparation",
        //     "What was discussed about follow-up meetings?",
        //     "Show me messages related to budget discussions",
        //     "Find conversations about meeting participants and attendance"
        // ];

        const queries = [
            "Can you share all mesages summery?"
        ];
        ...
        console.log("Main operation error:", error);
    } finally {
        process.exit(0);
    }
}
