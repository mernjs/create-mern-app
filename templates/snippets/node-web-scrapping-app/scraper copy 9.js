...
async function scrapeLocationEvents(page, location) {
    ...
    // Wait for the search to finish and page to load
    ...
    // Wait for initial set of events to load
    ...
    // Load more events by clicking the "More Events" button
    ...
    while (hasMoreEvents) {
        try {
            ...
            // Wait for the new events to load (adjust the timeout if needed)
            await delay(5000);

            // Collect more event links
            const newEventLinks = await page.evaluate(() => {
                return Array.from(document.querySelectorAll('a[data-testid="event-list-link"]')).map((link) => link.href);
            });

            // Check if new events are loaded
