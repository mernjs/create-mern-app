...
export function drawGrid(ctx, canvas) {
 ...
 // Update redrawLines function to handle potential issues with measurements array
export const redrawLines = (ctx, lines, measurements = []) => {
    lines.forEach((line, index) => {
      ...
      // Ensure the measurements array has the same length as lines
  ...
  export const update3DView = () => {
    const canvas = canvas3DRef.current;
    if (!canvas || !sceneRef.current || !cameraRef.current || !rendererRef.current) return;
    ...
    // Create and add new line meshes to the scene
    lines.forEach(line => {
