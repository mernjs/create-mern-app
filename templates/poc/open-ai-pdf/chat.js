...
async function createPdfAnalyzerAssistant(pdfPath) {
    try {
        const fileId = uploadFile(pdfPath)
...
const pdfPath = path.join(__dirname, "ExampleContractA13330013MB.pdf");
...
async function main() {
    try {
        ...
        await askAndGetResponse("What is ARTICLE 1 GENERAL PROVISIONS?");
